...
def test_KDTNode():
    ...
    """
    x = np.array([1, 4, 5, 6])
    nodex = nearest_neighbor.KDTNode(x)
    assert np.array_equal(nodex.value, x)
    assert nodex.left is None
    assert nodex.right is None
    assert nodex.pivot is None
    y = [1, 4, 5, 6]
    with pytest.raises(TypeError):
        nearest_neighbor.KDTNode(y)
