...
def count_sets(cards):
    ...
    """
    # counts the sets of 3 cards from 12 represented
    # by a list of string numbers 4 long in base 3

    if len(cards) != 12:
        raise ValueError("Hand must contain exactly 12 cards")
    ...
    for card in cards:
        if len(card) != 4:
            raise ValueError("Each card must have 4 digits")

    for card in cards:
        for ch in card:
            if ch not in "012":
                raise ValueError("Digits must be 0, 1, or 2")
...
def is_set(a, b, c):
    ...
    """
    # Returns True if a set is made from cards a, b, and c.
    # Otherwise returns False
    cards = [a, b, c]
    for i in range(4):
        matchornomatch = set([])
        for card in cards:
            matchornomatch.add(card[i])
        if len(matchornomatch) != 1 and len(matchornomatch) != 3:
            return False
    return True
